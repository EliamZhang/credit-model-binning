...
- 合箱主指标率：1M30+ / 3M30+ 金额逾期率（amt_bad_rate），用于单调性、相邻差异、
  候选评分与策略约束；
- 显著性检验：仍用 3M30+ 笔数两比例 Z 检验（两组人群"逾期结构"差异检验，统计上
  成立），作为合并代价中 (1-p) 项；
- 单箱样本量/成熟量/好坏样本量约束：仍用 3M30+ 笔数口径（样本充足性约束与指标
  口径无关）；
- IV：金额加权 IV（bad=逾期金额、good=敞口金额-逾期金额），与金额合箱目标一致。
...
4. 从 Train 初始箱出发，自动执行：约束修正 → 同时监控 1M30+ 与 3M30+ 金额逾期率
...
6. OOT 上验证最终方案：单调性、PSI（训练/OOT 总体及分月）、AUC、KS 和分段对比；输出
   月度稳定性矩阵；
7. 基于完整 Train 的最终箱统计，构建阈值曲线（累计/边际金额逾期率），在自动通过/人工审核/
...
9. 写入 out/binning_amt_strategy_report_YYYYMMDD.xlsx 并格式化（冻结窗格、列宽、条件色阶）。
...
DATA_DIR = Path("res")
OUT_DIR = Path("out")
REPORT_PATH = OUT_DIR / f"binning_amt_strategy_report_{time.strftime('%Y%m%d')}.xlsx"
...
# 合箱主指标（金额口径）：同时监控 1M30+ 和 3M30+ 金额逾期率。
PRIMARY_RATE_COLS = ["1m30p_amt_bad_rate", "3m30p_amt_bad_rate"]
PRIMARY_RATE_COL = "3m30p_amt_bad_rate"  # 主锚定指标（金额口径）；p 值仍用笔数两比例
# 混合口径：显著性检验与单箱样本量/成熟量/好坏量约束保持笔数口径（cnt）。
...
# 单调与相邻差异控制（金额口径；金额率量级约为笔数率 0.75~0.85 倍，
# 容差若过严可调小，如 0.002）。
...
# 默认策略的风险约束（金额口径）。
# 数值与笔数口径方案同风险水平（2026-08 数据：笔数版 auto 阈值处累计金额率
# 1M30+ 0.54% / 3M30+ 3.90%，accept 阈值处 1.10% / 5.97%；边际按档位率设定）。
# 金额版 7 档方案通过率离散（auto 可选 20.2%/50.0%，accept 可选 65.1%/85.3%），
# 且约束值会反作用于合箱保护边界（改约束即重排方案），无法精确对齐笔数版 40%/75%
# 通过率；经确认采用同风险约束：auto 落 B 档（通过率 20.2%）、accept 落 D 档（65.1%），
# 全部风险均低于笔数版方案（auto 2.73% vs 3.90%、accept 5.25% vs 5.97%）。
STRATEGY_CONFIG = {
