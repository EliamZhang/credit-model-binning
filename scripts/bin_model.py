...
用法：
    python scripts/bin_model.py --dataset laoke --model mlt
    python scripts/bin_model.py --dataset laoke --model worthiness
    python scripts/bin_model.py --dataset new --model new_mlt
...
def apply_configs(dataset_key: str, model_key: str) -> None:
    """把数据集/模型配置注入 settings，并同步到全部管线模块。"""
...
def resolve_report_path(dataset_key: str, model_key: str) -> Path:
    """输出 Excel 路径：前缀取自模型配置，文件名日期为运行当天。"""
    model_cfg = MODELS[model_key]
    return settings.OUT_DIR / f"{model_cfg['report_prefix']}_{time.strftime('%Y%m%d')}.xlsx"


def run(dataset_key: str = "laoke", model_key: str = "mlt") -> None:
    """按配置跑完整分箱管线并输出 Excel。"""
    apply_configs(dataset_key, model_key)
    report_path = resolve_report_path(dataset_key, model_key)
