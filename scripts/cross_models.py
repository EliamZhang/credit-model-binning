...
def resolve_report_path(dataset_key: str, model_a: str, model_b: str, mode: str) -> Path:
    ...
    if prefix is None:
        prefix = f"binning_cross_{model_a}_{model_b}_strategy_report"
...
def run(dataset_key: str = "laoke", model_a: str = "mlt", model_b: str = "worthiness", mode: str = "matrix") -> None:
    ...
    if mode == "matrix":
        ...
        raise ValueError(f"未知 mode: {mode}（可选 matrix）")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="两模型交叉分析：matrix 全局交叉")
    parser.add_argument("--dataset", default="laoke", choices=sorted(DATASETS), help="样本集 key")
    parser.add_argument("--model-a", default="mlt", choices=sorted(MODELS), help="A 模型 key（交叉的行轴）")
    parser.add_argument("--model-b", default="worthiness", choices=sorted(MODELS), help="B 模型 key")
    parser.add_argument("--mode", default="matrix", choices=["matrix"], help="分析模式")
    return parser
