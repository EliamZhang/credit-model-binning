...
def render_single_model(ctx: ReportContext, role: str = "a"):
    ...
    # 约束核对数值（从 03 的累计列取，与阈值选择一致；主口径按 metric 取金额/笔数率）
    auto_cum = tr_rows[auto_bin_rank(auto_bin) - 1]
    acc_cum = tr_rows[accept_bin_rank(accept_bin) - 1]
    auto_cum1 = auto_cum[f"cum_1m30p_{rate_sfx}_bad_rate"]
    auto_cum3 = auto_cum[f"cum_3m30p_{rate_sfx}_bad_rate"]
    acc_cum1 = acc_cum[f"cum_1m30p_{rate_sfx}_bad_rate"]
    acc_cum3 = acc_cum[f"cum_3m30p_{rate_sfx}_bad_rate"]
    ...
    A(f"# {ds_name}{model_cn}分数分箱与策略阈值设定报告（{metric_title}口径）\n")
    A(f"> 本报告说明{ds_name}{model_cn}（`{raw_score_col}`）分数分箱、样本外验证及策略阈值设定结果，由 `scr/_gen_new_reports.py` 从 `out/{xlsx.name}` 读取数值生成，与 Excel 逐项一致。管线沿用{rate_cn}违约合箱口径：完整 Train 用于学习分箱边界、执行合箱、选择候选方案和确定策略阈值；OOT 仅用于最终验证。")
    if include_y_cols:
    ...
    A("")
    A(f"| 约束阶段 | 累计 1M30+ {rate_cn}逾期率 | 累计 3M30+ {rate_cn}逾期率 | 边际 3M30+ {rate_cn}逾期率 |")
    A("| --- | --- | --- | --- |")
    A(f"| 自动通过 | ≤ {pct(auto_c[f'max_cum_1m30p_{rate_sfx}_bad_rate'])} | ≤ {pct(auto_c[f'max_cum_3m30p_{rate_sfx}_bad_rate'])} | ≤ {pct(auto_c[f'max_marginal_3m30p_{rate_sfx}_bad_rate'])} |")
    ...
    for i, trr in enumerate(tr_seg):
        oor = oo_seg[i]
        A(f"| {trr['decision']} | {pct(trr['strategy_estimated_segment_rate'])} | {pct(trr[f'3m30p_{rate_sfx}_bad_rate'])} | {pct(oor['strategy_estimated_segment_rate'])} | {pct(oor[f'3m30p_{rate_sfx}_bad_rate'])} | 风险梯度成立 |")
    A("")
    A("### （六）验证结论汇总\n")
    A("| 维度 | 结果 | 判定 |")
    A("| --- | --- | --- |")
...
def _read_app_info(ctx: ReportContext):
    ...
    with open(ROOT / ctx.dataset_cfg["data_dir"] / ctx.dataset_cfg["application_file"], encoding="utf-8-sig") as f:
        ...
        tix = hdr.index("application_time")
        six = hdr.index("status")
        fix = [hdr.index(x) for x in INCOME_FIELDS]
        for row in r:
            m = row[midx]
            if not m:
                m = row[tix][:7] if row[tix] else ""
            out[row[aidx]] = (m, [row[i] for i in fix], row[six] in DEAL_STATUSES)
    return out
...
def income_matrix_stats(ctx: ReportContext):
            ...
            for fi, f in enumerate(INCOME_FIELDS):
                ...
                if v not in ("null", "", "None"):
                    ...
                    if f in SURPLUS_FIELDS and fv >= 0:
                        pd[f].append(fv)
            if is_deal:
                dd = deal_acc.setdefault(key, {f: [] for f in SURPLUS_FIELDS})
                for fi, f in enumerate(INCOME_FIELDS):
                    if f not in SURPLUS_FIELDS:
                        continue
                    v = app[aid][1][fi]
                    if v not in ("null", "", "None"):
                        dd[f].append(float(v))
    ...
    deal_stats = {key: {f: _mean(vals) if vals else None for f, vals in d.items()} for key, d in deal_acc.items()}
...
def render_cross(ctx: ReportContext):
    ...
    B(">")
    B(f"> **一句话结论：不做分数融合；价值模型的正确用法是二维规则——AND 组合（{tag_a} ≤ {acc_a} 档且{disp_b} ≤ {acc_b} 档）可把接纳风险压低到 {pct(and_row['train_accept_3m30p'])}（{tag_a} 单模型接纳 {pct(a_row['train_accept_3m30p'])}；当前组合点接纳率 {pct(and_row['train_accept_rate'])}），而\"仅价值低\"的错配客群（Train {pct(quad[('train','仅wth低')]['sample_pct'])}、3M30+ {pct(quad[('train','仅wth低')]['3m30p_cnt_bad_rate'])}）必须由 {tag_a} 把关拦截。**")
    B("")
    B("## 一、结论摘要\n")
    ...
    for grp in ("train", "oot"):
        for key, label in [("双低", f"双低（{tag_a} ≤ {acc_a} 且 {tag_b} ≤ {acc_b}）"), ("仅mlt低", f"仅 {tag_a} 低（{tag_a} ≤ {acc_a} 且 {tag_b} > {acc_b}）"), ("仅wth低", f"仅 {tag_b} 低（{tag_b} ≤ {acc_b} 且 {tag_a} > {acc_a}）"), ("双高", f"双高（{tag_a} > {acc_a} 且 {tag_b} > {acc_b}）")]:
            r = quad.get((grp, key))
            if r:
