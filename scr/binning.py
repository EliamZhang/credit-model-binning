...
def format_rate(value):
    """坏账率默认使用 4 位小数展示。"""
    return format_pct(value, digits=4)
...
def append_scheme_comparison_table(md, schemes, schemes_oot, has_amount, higher_is_riskier):
    ...
    for scheme_name, s in schemes.items():
        reject_rule = (
            format_threshold_rule(s["review_max"], "reject", higher_is_riskier)
            if s["reject"]
            else "无"
        )
        line = (
            f"| {scheme_name} "
            f"| {format_threshold_rule(s['auto_max'], 'pass', higher_is_riskier)} "
            f"| {format_review_rule(s['auto_max'], s['review_max'], higher_is_riskier)} "
            f"| {reject_rule} "
            f"| {s['pass_rate']:.2%} "
            f"| {s['pass_bad_rate_count']:.4%} "
        )
        if has_amount:
            line += f"| {format_rate(s.get('pass_bad_rate_amount', float('nan')))} "
        line += f"| {s['reject_rate']:.2%} "

        if schemes_oot and scheme_name in schemes_oot:
            so = schemes_oot[scheme_name]
            line += f"| {so['pass_rate']:.2%} | {so['pass_bad_rate_count']:.4%} |"
        else:
            line += "| — | — |"
        md.append(line)
    md.append("")
...
def append_final_grade_overview(md, tuning_stats, oot_stats=None):
    ...
    for idx, row in enumerate(tuning_stats.itertuples()):
        oot_n = "—"
        oot_bad_rate = "—"
        if oot_stats is not None and idx < len(oot_stats):
            oot_row = oot_stats.iloc[idx]
            oot_n = format_int(oot_row["n"])
            oot_bad_rate = format_rate(oot_row["bad_rate"])
        md.append(
            f"| {idx + 1} "
            f"| {format_score_range(row.score_min, row.score_max)} "
            f"| {format_int(row.n)} "
            f"| {format_rate(row.bad_rate)} "
            f"| {format_pct(row.cum_pass_rate)} "
            f"| {oot_n} "
            f"| {oot_bad_rate} |"
        )
    md.append("")
...
def append_key_decision_section(
    md,
    schemes,
    schemes_oot,
    tuning_merged_stats,
    ...
    md.append("## 一、关键结论与推荐方案")
    md.append("")
    md.append(
        f"- 最终建议保留 **{len(tuning_merged_stats)} 个风险等级**；"
        f"调优集风险排序 Spearman ρ = {tuning_merged_rho:.4f}。"
    )
    if oot_merged_stats is not None:
        psi_text = f"，PSI = {merged_psi:.4f}" if merged_psi is not None else ""
        md.append(
            f"- OOT 集继续保持单调排序，AUC = {oot_auc:.4f}，KS = {oot_ks:.4f}{psi_text}。"
        )
    md.append(
        f"- 推荐常态使用 **平衡方案**：自动通过 {format_threshold_rule(recommended['auto_max'], 'pass', higher_is_riskier)}，"
        f"人工审核 {format_review_rule(recommended['auto_max'], recommended['review_max'], higher_is_riskier)}，"
        f"拒绝 {format_threshold_rule(recommended['review_max'], 'reject', higher_is_riskier)}。"
    )
...
# ---- ChiMerge 合并过程 ----
md.append("## 五、技术明细：ChiMerge 合并过程")
md.append("")
