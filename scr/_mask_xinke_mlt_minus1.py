...
背景（2026-09-01 用户确认）：xinke_mlt_score.csv 有 6.61%（37,987 笔）分数恰为 -1.0，
...
幂等：分数列已无 -1.0 时跳过写回；原文件备份为 xinke_mlt_score_nomask.csv（仅当不存在时）。
"""
import shutil
import sys
...
PATH = ROOT / "res" / "xinke_mlt_score.csv"
BACKUP = ROOT / "res" / "xinke_mlt_score_nomask.csv"
