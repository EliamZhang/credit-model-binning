...
def main():
    ...
    body(doc, "整体流程：在 Development 上等频初分 → 约束自动合箱为 6~8 档（目标 7 档）→ 候选方案评分择优 → 映射全部切片 → 风险约束下确定阈值 → OOT 独立验证 → 输出 Excel 策略报告。")
    flow_box(doc, [
    ...
    bullet(doc, "分箱边界仅在 Development 上学习，原样复用到 Validation、完整 Train 与 OOT，保证四切片口径一致。")
    bullet(doc, "Validation 模拟近期分布漂移，用于判断方案稳健性；OOT 全程不接触，作为最终“考卷”。")
    ...
    body(doc, "在 Development 上按分数分位数做 20 等频初分，区间规则 (left, right]，首尾扩展为 ±∞；分数重复过多时合并相同边界，实际箱数可能少于 20。自动合箱至 6~8 档，目标 7 档。")
    ...
    heading(doc, 2, "（三）单调性")
    bullet(doc, "Development：主指标（1M30+、3M30+ 笔数逾期率）相邻箱不允许倒挂（容差 0）。")
    bullet(doc, "Validation：允许不超过 0.3 个百分点的容忍倒挂。")
    bullet(doc, "候选评分同时监控 Validation 上四类风险率（含金额口径）的倒挂数。")
    ...
    body(doc, "每个阶段记录合并原因与前后状态，全过程可追溯；四阶段体现“先修病、再修型、后择优”的思路。")
    ...
    bullet(doc, "硬约束（必须全部满足）：档位数 6~8；Development 主指标无倒挂；单箱约束全部满足；未跨越极端圈选边界。")
    add_table(
        doc,
        ["评分项", "权重", "含义"],
        [
        ...
        footnote_lines=[
            "选择顺序：硬约束 → Development 倒挂数 → 约束违反数 → Validation 倒挂数 → PSI 可接受（≤ 0.10）→ PSI 偏好（≤ 0.05）→ 综合得分 → IV 保留率 → 档位距离，取第一名。",
    ...
    body(doc, "验证贯穿三个层次：Development 检查方案自身质量，Validation 检查跨期稳健性，OOT 作为独立数据做最终确认。")
