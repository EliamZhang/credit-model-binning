...
核心流程：
1. 加载并拼接样本、申请信息和主模型分，按月份切分 Train / OOT；
2. 在 Train 上进行 20 等频初分，将同一边界复用到全量样本；
3. 按配置将相邻初始箱合并为 A-E 最终风险等级；
4. 计算 1M30+ / 3M30+ 笔数及金额风险指标，检查单调性；
5. 使用 PSI、AUC、KS 做 Train / OOT 交叉验证；
6. 基于最终箱边界生成阈值曲线和自动通过 / 人工审核 / 拒绝策略；
7. 输出包含分箱过程、阈值选择依据和关键中间表的 Excel 报告。

运行方式：
    python binning.py

输入目录：res/
输出文件：out/策略报告_单策略版.xlsx
"""
