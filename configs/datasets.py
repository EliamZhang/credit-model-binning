...
DATASETS = {
    "laoke": {
        ...
        # 数据集级叙事（数值插值占位符：{n_miss_a}/{pct_a}/{n_miss_b}/{pct_b}）。
        "report_notes": {
            "maturity_note": "老客 Train 3M30+ 标签成熟率约 42.11%（check_data 完成申请口径，2026-09-07 复跑核对）",
            "cross_short_a": "mlt",
            "cross_short_b": "wth",
            "cross_missing_note": "mlt 缺失 {n_miss_a} 笔（{pct_a}）、价值模型缺失 {n_miss_b} 笔（{pct_b}，无银行交易数据人群）",
            "value_note_single": "价值语义说明：价值模型的本义为\"低分 = 高价值\"（分数越低，利息贡献越高）。老客数据经验验证方向一致——价值分与工资收入、借款本金显著负相关（corr 约 −0.42 / −0.37，收入随分数十分位单调下降），且高分同时为高风险。因此本报告的 A 档（最低分）同时是\"低风险 + 高价值\"档，G 档（最高分）同时是\"高风险 + 低价值\"档；风险类结论不受该语义影响，经营/提额类场景（优先经营象限、额度分层）需结合该语义使用（见《两模型交叉效果评估报告（mlt × 价值模型）》四象限与落地建议）。",
            "value_note_cross": "价值模型分为\"低分 = 高价值\"（分数越低，利息贡献越高）。因此价值档 A（最低分）同时是\"高价值 + 低风险\"档；下文\"价值 ≤ C\"即\"价值好（低分）人群\"。",
        },
    },
    "new": {
        "name": "新客",
